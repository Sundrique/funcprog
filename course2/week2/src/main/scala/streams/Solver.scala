...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    def streamNeighbors(neighbors: List[(Block, Move)]): Stream[(Block, List[Move])] = neighbors match {
      case Nil => Stream.empty
      case x :: xs => Stream.cons((x._1, x._2 :: history), streamNeighbors(xs))
    }

    streamNeighbors(b.legalNeighbors)
  }
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    case x #:: xs =>
      ...
      else Stream.cons(x, newNeighborsOnly(xs, explored + x._1))
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case x #:: xs =>
      val ns = for {
        neighbor <- newNeighborsOnly(neighborsWithHistory(x._1, x._2), explored)
      } yield neighbor
      x #:: from(xs append ns, explored + x._1)
  }
