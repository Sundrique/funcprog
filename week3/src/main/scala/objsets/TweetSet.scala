...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    def subTreeRetweets(st: TweetSet): Int =
      try {
        st.mostRetweeted.retweets
      } catch {
        case e: NoSuchElementException => -1
      }


    if (subTreeRetweets(left) > elem.retweets) left.mostRetweeted
    else if (subTreeRetweets(right) > elem.retweets) right.mostRetweeted
    else elem
  }
...
object GoogleVsApple {
  ...
  def googleFilter(tw: Tweet):Boolean = google.exists(tw.text.contains)
  def appleFilter(tw: Tweet):Boolean = apple.exists(tw.text.contains)
